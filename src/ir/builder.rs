...
impl<'a, 'nd, 'tcx> Builder<'a, 'tcx> {
    ...
    fn union_value(
        ...
        self.expr_arena
            .alloc(Expr::union_value(operand, tag, union_ty))
    }
    #[inline]
                    ...
                    Type::Union(operand_member_types) => {
                        ...
                        let union_member_value = self.build_union_members_mapped_value(
                            ...
                            |member_value, _| {
                                ...
                                let access = self.tuple_index_access(member_value, index);
    ...
    /// Type promotion.
    /// Assuming that the type check has already been passed, convert
    /// the representation of the value to the expected type.
    fn promote_to(
        &self,
        operand: &'a Expr<'a, 'tcx>,
        ...
        self._promote_to(operand, expected_ty, &mut vec![], tmp_vars, stmts)
    }
    ...
    fn _promote_to(
        ...
        operand: &'a Expr<'a, 'tcx>,
        expected_ty: &'tcx Type<'tcx>,
        non_matched_conditions: &mut Vec<&'a Expr<'a, 'tcx>>,
        tmp_vars: &mut TmpVars<'a, 'tcx>,
        stmts: &mut Vec<&'a Stmt<'a, 'tcx>>,
    ) -> &'a Expr<'a, 'tcx> {
        ...
        match (operand_ty, expected_ty) {
            (Type::Union(operand_member_types), Type::Union(expected_member_types)) => {
                ...
                let union_member_value = self.build_union_members_mapped_value(
                    ...
                    union_tag,
                    expected_ty,
                    non_matched_conditions,
                    |member_value, non_matched_conditions| {
                        ...
                        let member_value = self._promote_to(
                            member_value,
                            expected_member_ty,
                            non_matched_conditions,
                            tmp_vars,
                        ...
                        &*self.push_expr_alloc(
                            Expr::union_value(member_value, expected_tag, expected_ty),
                            tmp_vars,
                            stmts,
                        )
                    },
            ...
            (operand_ty, Type::Union(expected_member_types)) => {
                ...
                for (tag, expected_member_type) in expected_member_types.iter().enumerate() {
                    // Found assignable member, return promoted value.
                    if operand_ty.is_assignable_to(expected_member_type) {
                        let value = self._promote_to(
                            operand,
                        ...
                        return self.push_expr(
                            self.union_value(value, tag, expected_ty),
                            tmp_vars,
                            stmts,
                        );
                    }
                    // Types are compatible with each other but cannot be directly assigned to each other,
                    // so we must selectively convert the operand value.
                    else if expected_member_type.is_assignable_to(operand_ty) {
                        let mut non_matched_conditions = vec![];

                        let promoted = self._promote_to(
                            operand,
                            expected_member_type,
                            &mut non_matched_conditions,
                            tmp_vars,
                            stmts,
                        );

                        let cond = non_matched_conditions
                            .iter()
                            .map(|cond| self.not(cond))
                            .reduce(|lhs, rhs| self.and(lhs, rhs));

                        let union_value = self.push_expr(
                            self.union_value(promoted, tag, expected_ty),
                            tmp_vars,
                            stmts,
                        );

                        if let Some(cond) = cond {
                            cond_values.push((cond, union_value));
                        } else {
                            return union_value;
                        }
                    }
                ...
                if !cond_values.is_empty() {
                    return cond_values.iter().rev().skip(1).fold(
                        cond_values.last().unwrap().1,
            ...
            (Type::Struct(operand_struct_ty), Type::Struct(expected_struct_ty)) => {
                ...
                let promoted_fields = expected_struct_ty
                    ...
                    .map(|f| {
                        ...
                        let value =
                            self._promote_to(expr, f.ty(), non_matched_conditions, tmp_vars, stmts);
            ...
            (Type::Tuple(operand_fs), Type::Tuple(expected_fs)) => {
                ...
                let m = expected_fs
                    ...
                    .map(|(i, fty)| {
                        ...
                        let expr = self.push_expr_alloc(expr, tmp_vars, stmts);
                        self._promote_to(expr, fty, non_matched_conditions, tmp_vars, stmts)
                    })
                    .collect();
    ...
    fn build_pattern(
        ...
        // annotation, the value is checked for type conformity, but isn't
        // converted to that type. The value will be converted when it is bound to
        // a variable.
        let ty_cond = pat
            .explicit_ty()
            .and_then(|explicit_ty| self._build_pattern_type_test(target_expr, explicit_ty));
        let pat_cond = self._build_pattern(target_expr, pat, tmp_vars, program, outer_stmts, stmts);
        self.and_some(ty_cond, pat_cond)
    }
    fn _build_pattern_type_test(
        ...
        match (target_ty, pat_ty) {
            ...
            | (&Type::LiteralInt64(n), Type::Int64 | Type::NativeInt) => {
                Some(self.eq(target_expr, self.int64(n)))
            }
            (Type::Boolean, &Type::LiteralBoolean(b))
            | (&Type::LiteralBoolean(b), Type::Boolean) => {
                let cond = if b {
                    target_expr
                } else {
            ...
            (Type::Tuple(target_fs), Type::Tuple(pat_fs)) => {
                ...
                pat_fs
                    ...
                    .filter_map(|(i, pat_ty)| {
                        ...
                        self._build_pattern_type_test(operand, pat_ty)
                    })
                    .reduce(|lhs, rhs| self.and(lhs, rhs))
            }
            (Type::Struct(_), Type::Struct(pat_struct_ty)) => pat_struct_ty
                ...
                .filter_map(|field| {
                    ...
                    self._build_pattern_type_test(operand, field.ty())
                })
                .reduce(|lhs, rhs| self.and(lhs, rhs)),
            ...
            (_, Type::Union(_)) => {
                // The type of the target value is not a union type and the type of
                // the pattern is a union type.
                None
            }
            ...
            (Type::Named(_), _) | (_, Type::Named(_)) => {
                unreachable!("bug: named type must be resolved.")
            }
            (Type::Undetermined, _) | (_, Type::Undetermined) => {
                unreachable!("bug: type must be inferred.")
            }
            (Type::LiteralInt64(n1), Type::LiteralInt64(n2)) if n1 == n2 => None,
            (Type::LiteralBoolean(n1), Type::LiteralBoolean(n2)) if n1 == n2 => None,
            (Type::LiteralString(n1), Type::LiteralString(n2)) if n1 == n2 => None,
            _ => unreachable!("bug: types must be compatible."),
        }
    }
    fn _build_single_pattern(
        &self,
        target_expr: &'a Expr<'a, 'tcx>,
        pat: &'nd syntax::Pattern<'nd, 'tcx>,
        tmp_vars: &mut TmpVars<'a, 'tcx>,
        ...
        match pat.kind() {
            &PatternKind::Integer(value) => {
                if let &Type::LiteralInt64(ty_value) = target_expr.ty() {
                    if value == ty_value {
                        None
                    } else {
                        // Returns `false` condition because literal type of
                        // different values don't match each other.
                        Some(self.bool(false))
                    }
                } else {
                    Some(self.eq(target_expr, self.int64(value)))
                }
            }
            &PatternKind::Boolean(value) => {
                if let &Type::LiteralBoolean(ty_value) = target_expr.ty() {
                    if value == ty_value {
                        None
                    } else {
                        // Returns `false` condition because literal type of
                    ...
                    Some(self.not(target_expr))
                }
            }
            PatternKind::String(value) => {
                if let Type::LiteralString(ty_value) = target_expr.ty() {
                    if value == ty_value {
                        None
                    } else {
                        // Returns `false` condition because literal type of
            ...
            &PatternKind::Range { lo, hi, end } => {
                if let &Type::LiteralInt64(ty_value) = target_expr.ty() {
                    if match end {
                        syntax::RangeEnd::Included => ty_value >= lo && ty_value <= hi,
                        syntax::RangeEnd::Excluded => ty_value >= lo && ty_value < hi,
                    } {
                        None
                    } else {
                        // Returns `false` condition because the value of literal type is
                        // outside of the pattern range.
                        Some(self.bool(false))
                    }
                } else {
                    ...
                    let lhs = self.ge(target_expr, lo);
                    let rhs = match end {
                        syntax::RangeEnd::Included => self.le(target_expr, hi),
                        syntax::RangeEnd::Excluded => self.lt(target_expr, hi),
                    };
            ...
            PatternKind::Struct(struct_pat) => {
                ...
                if let Some(spread_pat) = spread_pat {
                    if let Some(spread_name) = spread_pat.name() {
                        ...
                        stmts.push(self.var_def_stmt(spread_name.to_string(), struct_value));
                    }
                }
                ...
                pattern_fields
                    ...
                    .filter_map(|pat_field| {
                        let operand = self.struct_field_access(target_expr, pat_field.name());
                        self.build_pattern(
                            operand,
                            pat_field.pattern(),
                            tmp_vars,
            ...
            PatternKind::Var(_) | PatternKind::Wildcard => {
                unreachable!("these pattern should be handled in _build_pattern")
            }
            PatternKind::Or(_) => unreachable!("not a single pattern"),
        }
    }
    fn _build_pattern(
        &self,
        ...
        match pat.kind() {
            PatternKind::Var(name) => {
                ...
                let value_expr = self.promote_to(target_expr, pat_ty, tmp_vars, stmts);
                stmts.push(self.var_def_stmt(name.to_string(), value_expr));
                None
            }
            ...
            PatternKind::Or(sub_pats) => {
                ...
                let pat_cond = sub_pats
                    ...
                    .map(|sub_pat| {
                        ...
                        self.cond_and_assign(sub_cond, cfv)
                    })
                    .reduce(|lhs, rhs| self.or(lhs, rhs));
    ...
    /// a union type operand. A conversion function can be specified.
    ///
    /// Assuming that the type check has already been passed, the tag check in
    /// this method allows compatible types such as literal type and base type.
    ...
    where
        F: FnMut(&'a Expr<'a, 'tcx>, &mut Vec<&'a Expr<'a, 'tcx>>) -> &'a Expr<'a, 'tcx>,
    {
        ...
        let vs: Vec<_> = operand_member_types
            ...
            .filter_map(|(tag, member_ty)| {
                ...
                let value = member_value_mapper(member_value, non_matched_conditions);

                // Incompatible typed value should be dropped.
                if value.ty().is_compatible(expected_ty) {
                    Some((&*cond, value))
                } else {
                    non_matched_conditions.push(cond);
                    None
                }
            })
            .collect();
        ...
        assert!(
            !vs.is_empty(),
            "no matched type in {:?}, expected type = {:?}",
            operand_member_types,
            expected_ty
        );
